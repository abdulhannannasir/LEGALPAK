...
import { OBLIGATION_STATUS_STYLE } from "./obligation-badge";
...
/**
 * A plain month grid for the unified Compliance Center calendar — no
 * calendar library. Date math is all Date.UTC / 'YYYY-MM-DD' strings, the
 * same timezone-safe convention as src/lib/legal/date.ts, so "today" and
 * each cell line up with due_date without an off-by-one for a Pakistan
 * (UTC+5) reader.
 */
export function UnifiedComplianceCalendar({ items }: { items: UnifiedComplianceItem[] }) {
  const now = new Date();
  const [cursor, setCursor] = useState({ year: now.getUTCFullYear(), month: now.getUTCMonth() });

  const itemsByDate = useMemo(() => {
    const map = new Map<string, UnifiedComplianceItem[]>();
    ...
    <div className="rounded-[var(--radius-lg)] border border-border bg-surface p-4">
                  ...
                  <div className="mt-1 space-y-0.5">
                    {dayItems.slice(0, 3).map((item) => (
                      <Link
                        ...
                        className={`block truncate rounded border px-1 py-0.5 text-[10px] font-medium ${OBLIGATION_STATUS_STYLE[item.status].badge}`}
                      >
                        {item.title}
                      </Link>
                    ))}
      ...
      {unscheduled.length > 0 && (
        <p className="mt-3 text-xs text-muted">
          + {unscheduled.length} item{unscheduled.length === 1 ? "" : "s"} with no due date — configuration required —
