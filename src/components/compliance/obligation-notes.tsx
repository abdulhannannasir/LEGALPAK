...
import {
  addObligationNoteFn,
  deleteObligationNoteFn,
  listObligationNotesFn,
  type ComplianceObligationNote,
...
/** Free-form, workspace-shared notes on a compliance obligation — the "Add Note" action from the obligation detail page. */
export function ObligationNotes({ obligationId }: { obligationId: string }) {
  const [notes, setNotes] = useState<ComplianceObligationNote[] | null>(null);
  ...
  function refresh() {
    setLoadError(false);
    listObligationNotesFn({ data: obligationId })
      .then(setNotes)
      .catch(() => {
        setLoadError(true);
  ...
  useEffect(refresh, [obligationId]);

  async function submit() {
    if (!text.trim()) return;
    setSaving(true);
    try {
      await addObligationNoteFn({ data: { obligationId, body: text.trim() } });
      setText("");
      refresh();
    } catch {
  ...
  async function remove(id: string) {
    setBusyId(id);
    try {
      await deleteObligationNoteFn({ data: id });
      refresh();
    } catch {
  ...
  return (
    <section className="rounded-[var(--radius-lg)] border border-border bg-surface p-5">
      ...
      <p className="mt-1 text-sm text-muted">
        Keep context on this obligation — who's handling it, what's blocking it, what changed.
      </p>
