import { addDaysISO, addMonthsISO, formatLong, formatShort, isAfterISO } from "./date.ts";
...
export function classifyAccounts(input: AccountsInput): AccountsAdvice {
  ...
  const agmDue = input.fyEnd ? addDaysISO(input.fyEnd, 120) : null;
  const firstAgmDue = input.incorporationDate ? addMonthsISO(input.incorporationDate, 16) : null;
  const accountsDue =
    fileWithSecp && input.agmDate && filingDaysAfterAgm
      ? addDaysISO(input.agmDate, filingDaysAfterAgm)
      : null;
  const formADue = input.agmDate ? addDaysISO(input.agmDate, 30) : null;

  if (input.agmDate && agmDue && isAfterISO(input.agmDate, agmDue)) {
    flags.push({
      level: "high",
  ...
  if (input.agmDate && firstAgmDue && isAfterISO(input.agmDate, firstAgmDue)) {
    flags.push({
      level: "high",
      title: "First AGM beyond 16 months of incorporation",
      detail: `First AGM due by ${fmt(firstAgmDue)}.`,
    });
  }
