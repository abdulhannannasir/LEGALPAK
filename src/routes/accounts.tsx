  ...
  head: () => ({
    meta: [
      { title: "Financial statements — LegalPak" },
      {
        name: "description",
        content:
          "Classify audit vs. SECP filing requirements, track the 15- and 30-day clocks, and draft a board resolution and directors' report skeleton for your Pakistani company.",
      },
    ],
  }),
});
...
function AccountsPage() {
  const [form, setForm] = usePersistedState<AccountsInput>("legalpak:accounts", empty);
  ...
  function generate() {
    ...
    if (!form.fyEnd || !form.agmDate) {
      toast.error("Set the financial year end and AGM date for accurate deadlines");
      return;
    }
    setShow(true);
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted">
          Classifies audit, directors’ report and whether the pack must go to the registrar.
          Generates a board resolution and eZfile memo. Does not submit to SECP.
        </p>
      </div>
        ...
        preview={
          show ? (
              ...
              <section className="rounded-[var(--radius-lg)] border border-border bg-surface p-5">
                ...
                <dl className="mt-4 grid gap-3 sm:grid-cols-2 text-sm">
                  ...
                  <div>
                    ...
                    <dd className="font-medium">
                      {advice.directorsReport ? "Required" : "Exempt"}
                    </dd>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <dd className="font-medium">
                      {advice.accountsDue ?? "Not a registrar filing"}
                    </dd>
                  </div>
                  <div>
            ...
            <section className="rounded-[var(--radius-lg)] border border-dashed border-border p-8 text-center text-sm text-muted">
              Fill in the company name, financial year end and AGM date, then classify to see the
              pack here.
            </section>
          )
        }
